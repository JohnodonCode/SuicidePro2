...
using SuicidePro2.API.Extensions;
using SuicidePro2.Handlers.CustomEffectHandlers;
using System;
...
namespace SuicidePro2.Handlers
{
    public static class Methods
    {
        ...
        {
            SuicidePro2.Instance.Coroutines.Add(Timing.RunCoroutine(_runConfig(config, player)));
        }
        ...
        public static void RunCommands(this List<string> commands, Player player = null)
        {
            foreach (var cmd in commands)
                GameCore.Console.singleton.TypeCommand(cmd.Replace("%playerid%", player.PlayerId.ToString()));
        }
        ...
        private static IEnumerator<float> _runConfig(Config.BaseCommandConfig config, Player player)
        {
            ...
            switch (config)
            {
                ...
                case Config.CustomHandlerCommandConfig cCfg:
                    ...
                    player.Hurt(handler);
                    handler.StartVelocity = player.Velocity + cCfg.DamageHandler.Velocity.ToVector3(player.Camera.transform);
