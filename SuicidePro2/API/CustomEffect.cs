...
namespace SuicidePro2.API
{
    public abstract class CustomEffect
    {
        ...
        public void Register(IgnoreRequirementType force = IgnoreRequirementType.None)
        {
            ...
            {
                Log.Warning($"{this} attempted to register but failed: already registered.");
                return;
            }

            if (Effects.Any(x => x.Id == Id) && !force.HasFlag(IgnoreRequirementType.IdDuplicates))
            {
                Log.Warning($"{this} attempted to register but failed: another effect already uses this ID.");
                return;
            }
            ...
            Effects.Add(this);
            Log.Debug($"{this} registered successfully!", SuicidePro2.Instance.Config.Debug);
        }
